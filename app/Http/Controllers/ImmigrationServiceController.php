...
class ImmigrationServiceController extends Controller
{
    ...
    public function showImmigrationEditPage()
    {
        $service = ImmigrationService::where([
            ['status', '=', true],
            ['id', '=', request()->id]
        ])->first()->with([
            'highlights',
    ...
    public function editImmigrationService(Request $request)
    {
        try {
            $service = ImmigrationService::find($request->id);
            ...
            // Inserting Option 1 Data
            $service->option_1_title = $request->options_1_title;
            ...
            ImmigrationHighlight::where('feature_id', $service->id)->delete();
            ImmigrationBenefit::where('benefit_id', $service->id)->delete();
            ImmigrationRequirement::where('requirement_id', $service->id)->delete();
            ImmigrationSecondRequirement::where('requirement_id', $service->id)->delete();
            ImmigrationRequirement::where('requirement_id', $service->id)->delete();
            ImmigrationTimelineOfEvent::where('timeline_id', $service->id)->delete();
            ...
            if ($service->highlight_features_active == true) {
                ...
                for ($i = 0; $i < count($highlight_features); $i++) {
                    ...
                    $newFeature->feature_title = $request->highlight_feature[$i];
                    $newFeature->feature_context = $request->highlight_context[$i];
                    $newFeature->feature_id = $service->id;
                    $newFeature->save();
                }
            }
